...
export class VentaComponent implements OnInit {
  ...
  retornarProductosPorFiltro(busqueda:any):Producto[]{
    ...
    return this.listaProductos.filter(item => item.nombre.toLocaleLowerCase().includes(valorBuscado) && item.stock > 0);
  }
  ...
  agregarProductoParaVenta(){
    ...
    }else if (this.totalPagar + _total > this.productoSeleccionado.stock * _precio) {
      Swal.fire({
        icon: 'error',
        title: 'Cantidad excede el stock disponible',
        text: 'La cantidad ingresada para la venta es mayor que el stock disponible para este producto.'
      }); 
    }else{
