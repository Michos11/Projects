...
export class HistorialVentaComponent implements OnInit, AfterViewInit {
  ...
  //   this._ventaServicio.obtenerDatosIniciales().subscribe({
  //     next: (data) => {
  //       if (data.status) {
  //         this.datosListaVenta = new MatTableDataSource<Venta>(data.value);
  //       } else {
    ...
    this._ventaServicio.historial(
      ...
      next: (data) =>{
        ...
        if (data.status) {
          ...
          this.datosListaVenta.filter = this.formularioBusqueda.value.numero.trim().toLowerCase();
        } else {
          this._utilidadServicio.mostrarAlerta("No se encontraron datos", "Oops!");
        }
      },
